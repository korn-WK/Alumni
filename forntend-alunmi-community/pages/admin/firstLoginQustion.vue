    ...
    <v-card class="pa-8 elevation-2 rounded-xl">
      ...
      <v-data-table :headers="headers" :items="questionnaires" sort-by="color">
        ...
        <template #[`item.choice`]="{ item }">
          <div
            v-for="(itemChoice, indexChoice) in item.choice"
            :key="indexChoice"
          >
...
<script>
const API_URL = `${process.env.baseUrl}/questionaire/firstlogin`
  ...
  methods: {
    ...
    async addQuestionnaire() {
      try {
        ...
        await this.$axios.post(API_URL, newQuestionnaire)
        this.dialog = false
        this.fetchQuestionnaires()
        this.clearFields()
      } catch (error) {
