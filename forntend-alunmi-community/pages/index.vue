...
<template>
  ...
  <div class="Home-bg">
    ...
    <div>
      <div 
      v-for="(butterfly, index) in butterflies" :key="index" class="butterfly" :style="{
    ...
    <div class="text-center">
      <v-row align="center" justify="center">
        ...
        <span style="margin-left: 10px;">{{ visitCount }} Users</span>
      </v-row>
    </div>
    ...
    <v-footer v-if="showCookieBanner" app fixed style="padding: 0; margin: 0; background-color: #f8f9fa;">
      <v-col class="text-center" cols="12" style="padding: 0; margin: 0;">
        <div class="py-3" style="background-color: #e9ecef; padding: 0; margin: 0; color: #333; font-family: 'Roboto', sans-serif; font-size: 16px;">
          <p style="margin-bottom: 16px; line-height: 1.6; text-shadow: none;">
            ...
            <br />
            <a href="https://pdpa.mfu.ac.th/pdpa-6250.html" target="_blank" style="color: #0056b3; text-decoration: underline; font-weight: bold;">
              ประกาศเกี่ยวกับความเป็นส่วนตัว
            </a>&nbsp;
            <a href="https://pdpa.mfu.ac.th/papa-6240/pdpa-6251.html" target="_blank" style="color: #0056b3; text-decoration: underline; font-weight: bold;">
              นโยบายการใช้คุกกี้
            </a>
          </p>
          <v-btn class="mb-2"  color="#ae1b1f" dark style="font-size: 14px; padding: 10px 20px; font-weight: bold;" @click="handleAccept">Accept</v-btn>
        </div>
      </v-col>
    </v-footer>
    ...
    <!-- Main Footer -->
    <v-footer v-else class="footer">
      <v-row style="color: #f5be46;" justify="center" no-gutters>
        <!-- Facebook Icon -->
        <v-col cols="auto">
          <a href="https://www.facebook.com/profile.php?id=100082801136789" target="_blank" rel="noopener noreferrer">
    ...
    <!-- dialog Alumni -->
    <v-dialog v-model="dialogAlumni" content-class="elevation-0" max-width="1140px">
...
export default {
  ...
  computed: {
    firstLogin() {
      return this.$auth.user?.firstLogin; // อย่ายุ่ง
    },
    filteredAlumniPosts() {
  ...
  methods: {  // อย่ายุ่ง
    ...
    acceptCookies() {
      this.showCookieBanner = false; // Hide the cookie banner
    },

    async created() {
    try {
      const response = await this.$axios.$get(`${process.env.baseUrl}/visits/count`);
      if (!response.ok) throw new Error("Failed to fetch visit count");
      this.visitCount = response.data.totalVisits;
    } catch (error) {
    ...
    async updateVisitCount() {
      try {
        ...
        const incrementResponse = await this.$axios.$post(`${process.env.baseUrl}/visits/increment`);
        console.log("Increment Response:", incrementResponse);
        this.visitCount = incrementResponse.totalVisits;

        // Fetch updated visit count
        const countResponse = await this.$axios.$get(`${process.env.baseUrl}/visits/count`);
        console.log("Count Response:", countResponse);
        this.visitCount = countResponse.totalVisits;
      } catch (error) {
    ...
    async fetchAlumniData() { // อย่ายุ่ง
      try {
        const response = await this.$axios.$get(`${process.env.baseUrl}/postAlumnis`);
    ...
    async fetchAdminData() { // อย่ายุ่ง
      try {
        const response = await this.$axios.$get(`${process.env.baseUrl}/postAdmins`);
    ...
    checkStatusDialog() {  // อย่ายุ่ง
      if (this.firstLogin === 0) {
        this.dialogQuestion = true;
      }
    },
    handleClick(topic) { // อย่ายุ่ง
    ...
    showDialog(item) { // อย่ายุ่ง
      this.dialogItem = { ...item, img: item.img };
      this.dialogAdmin = true;
    },
    showAlumniDialog(item) { // อย่ายุ่ง
    ...
    addTodo() {
      ...
      if (this.todoText.trim()) {
        ...
        this.todos.push({ text: this.todoText, timestamp });  // Add message with timestamp
    ...
    // Edit a todo
    editTodo(index) {
      const newText = prompt('Edit your message:', this.todos[index].text);
      if (newText && newText !== this.todos[index].text) {
        this.todos[index].text = newText; // Update the message text
      }
    },
